...
if (! class_exists('TGM_Plugin_Activation'))
{
	...
	class TGM_Plugin_Activation
	{
		...
		public function load_textdomain(): void
		{
			...
			if (false !== mb_strpos(__FILE__, WP_PLUGIN_DIR) || false !== mb_strpos(__FILE__, WPMU_PLUGIN_DIR))
			{
		...
		public function overload_textdomain_mofile($mofile, $domain)
		{
			...
			if ('tgmpa' !== $domain || false === mb_strpos($mofile, WP_LANG_DIR) || @is_readable($mofile))
			{
				return $mofile;
			}

			// Current fallback file is not valid, let's try the alternative option.
			if (false !== mb_strpos($mofile, '/themes/'))
			{
				return str_replace('/themes/', '/plugins/', $mofile);
			}
			if (false !== mb_strpos($mofile, '/plugins/'))
			{
		...
		protected function get_admin_notice_class()
		{
			...
			{
				return sanitize_html_class(mb_strtolower($this->strings['nag_type']));
			}
		...
		protected function is_core_update_page()
		{
			...
			if ('update' === $screen->base && ! empty($_POST['action']))
			{ // WPCS: CSRF ok.
